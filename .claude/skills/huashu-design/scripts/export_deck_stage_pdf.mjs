...
async function main() {
  ...
  console.log(
    `Rendering ${path.basename(htmlAbs)} → ${path.basename(outFile)}`,
  );
  ...
  const sectionCount = await page.evaluate(
    ({ W, H }) => {
      ...
      if (!stage)
        throw new Error(
          "<deck-stage> not found — 这个脚本只适用于单文件 deck-stage 架构",
        );
      const sections = Array.from(stage.querySelectorAll(":scope > section"));
      if (!sections.length)
        throw new Error("No <section> found inside <deck-stage>");
  ...
  console.log(
    `  验证页数：mdimport "${outFile}" && pdfinfo "${outFile}" | grep Pages`,
  );
}
